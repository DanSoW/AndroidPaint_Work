...
public class Sprite {
    ...
    public boolean clicked(float coordX, float coordY, int rate){
        return ((coordY >= (getBoundingBoxRect().top - rate))
                && (coordY <= (getBoundingBoxRect().bottom + rate))
                && (coordX >= (getBoundingBoxRect().left - rate))
                && (coordX <= (getBoundingBoxRect().right + rate)));
    }
}